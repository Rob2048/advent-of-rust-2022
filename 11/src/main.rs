    ...
    for round in 0..3 {
        ...
        for monkey_index in 0..monkeys.len() {
            ...
            for item in &monkey.items {
                ...
                let limit = monkey.test_operand * monkey.test_operand;

                let mut new_item_worry = match monkey.inspect_operation {
                    InspectOperation::AddOperand => new_item + monkey.inspect_operand,
                    InspectOperation::MultiplyOperand => new_item * monkey.inspect_operand,
                    InspectOperation::AddOld => new_item + new_item,
                    InspectOperation::MultiplyOld => new_item * new_item,
                };

                // if new_item_worry >= limit {
                //     new_item_worry %= limit;
                // }

                // if new_item_worry >= monkey.test_operand
        ...
        // if round == 0 || round == 19 || round % 1000 == 0 {
            ...
            for (index, monkey) in monkeys.iter().enumerate() {
                println!(
                    ...
                    index, monkey.total_inspections
                );
            }
            println!();
        // }
